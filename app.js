...
function initCheckout() {
  ...
  function openCheckoutStep(stepNumber) {
    ...
    // Address elements adjustments:
    // If cart contains only digital courses, hide Shipping Address inputs
    ...
    // Payment screen totals calculation
    if (stepNumber === 2) {
      const subtotal = cart.reduce((sum, item) => sum + (item.price * item.qty), 0);
      const shipping = hasPhysical ? 0 : 0; // free shipping template
